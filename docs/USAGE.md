...
## Before Submitting — Human Review Checklist

> **dragonflAI enforces this checklist at the API and UI level.** Export and submission
> are blocked until a finding has been marked as human-reviewed.

All findings — whether created manually, auto-generated by detection, or promoted from scan results — **start as `draft`** and must pass through an explicit human review before they can be exported or submitted.

### Review steps

1. **Open the Finding in the UI** — navigate to *Findings → Finding Detail & Reports*.
2. **Confirm scope** — verify the affected asset and endpoint are within the program's defined scope. Out-of-scope submissions are rejected and may violate program rules.
3. **Confirm impact and reproduction** — reproduce the vulnerability manually. Ensure steps to reproduce are accurate, complete, and reproducible by a third party.
4. **Confirm rate-limit compliance** — verify your testing did not exceed the program's stated rate limits. Do not submit findings obtained through excessive automated scanning.
5. **Confirm no sensitive data was stored** — check that no credentials, PII, or other sensitive data from the target appear in the finding description, notes, or evidence paths.
6. **Check the review checkbox in the UI** — tick *"I have reviewed this finding and confirmed it complies with program rules"* and enter your reviewer name.
7. **Export the report** — export is now enabled; download the Markdown or plain-text report.
8. **Submit to the platform** — paste or attach the exported report to the bug bounty platform submission form.
...
| Status | Description |
|---|---|
| `draft` | Initial state. All auto-generated and promoted findings start here. |
| `needs_review` | Set automatically when the first human review is recorded. |
| `ready_to_submit` | Set manually when the finding is ready; requires `reviewed_by_human=true`. |
| `submitted` | Set manually after submission to the platform; requires `reviewed_by_human=true`. |
| `accepted` | Optional — record when the platform accepts the finding. |
| `duplicate` | Optional — record when the platform marks it as a duplicate. |
| `not_applicable` | Optional — record when the finding is out of scope or invalid. |

### API enforcement

If you use the API directly, the following endpoints enforce human review:

```bash
# Mark a finding as reviewed (required before export/submit)
curl -X POST http://127.0.0.1:8000/findings/42/review \
  -H "Content-Type: application/json" \
  -d '{"reviewer": "alice", "review_notes": "Verified manually — confirmed open redirect."}'

# Advance status to ready_to_submit (only allowed after review)
curl -X POST "http://127.0.0.1:8000/findings/42/status?status=ready_to_submit"

# Export report (blocked without human review)
curl "http://127.0.0.1:8000/findings/42/export?format=markdown" -o submission.md
```

Attempting to export or update status to `ready_to_submit`/`submitted` without calling `/review` first returns a **400 error** with a message explaining the requirement.
